...
$pilihres    = mysqli_query($conn,"SELECT * FROM t_reservoir ORDER BY id_res DESC");	
	while ($rowres = mysqli_fetch_array($pilihres)){
		$pilihret    = mysqli_query($conn,"SELECT * FROM t_p_retikulasi WHERE id_ret=$rowres[id_res] ORDER BY id_ret DESC");
		while ($rowret      = mysqli_fetch_array($pilihret)){
		...
		"pipa_dinas_awal"=>$rowdin['debit_dinas_1'],
		"pipa_dinas_akhir"=>$rowdin['debit_dinas_2'],
		"tekanan_reservoir"=>$rowres['sens_tekanan'],
...
{	
	...
	$sqlret   = "INSERT INTO t_p_retikulasi 
				...
				( 'id_ret', '$debit_retikulasi1', '$debit_retikulasi2','-6.884023', '107.551895' , '$waktu' , '$tanggal' );";
	
	$sqldin   = "INSERT INTO t_p_dinas 
				(id_pdinas,debit_dinas_1,debit_dinas_2,latitude,longitude,waktu,tanggal) VALUES 
				( 'id_pdinas', '$debit_dinas1', '$debit_dinas2','-6.883389', '107.551268' , '$waktu' , '$tanggal' );";
