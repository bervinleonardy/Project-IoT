...
$pdf->Cell(200,5,'DATA SENSOR DI RESERVOIR',0,1,'C');

$pdf->SetFont('Arial','',9);
$pdf->Cell(70,8,'Data Reservoir Portal WLDS Sistem Monitoring Kota Cimahi',0,1,'L'); // cell dengan panjang 1
$pdf->ln(0.3);
$pdf->SetFont('Arial','',9);
$pdf->Cell(10,10,'NO',1,0,'C');
$pdf->Cell(40,10,'Tekanan Air Reservoir',1,0,'C');
$pdf->Cell(40,10,'Volume Reservoir',1,0,'C');
$pdf->Cell(40,10,'Waktu',1,0,'C');
$pdf->Cell(40,10,'Tanggal',1,0,'C');
$pdf->Ln();

$pdf->SetFont('Arial','',8);
$cari_p = mysqli_query($conn, "SELECT * FROM t_reservoir");
$no=1;
while ($data = mysqli_fetch_array($cari_p)){
	$pdf->Cell(10,5,$no++,1,0,'C');
	$pdf->Cell(40,5,$data['sens_tekanan'].' PSi',1,0,'C');
	$pdf->Cell(40,5,$data['sens_ultrasonic'].' Liter',1,0,'C');
	$pdf->Cell(40,5,$data['waktu'],1,0,'C');
	$pdf->Cell(40,5,$data['tanggal'],1,0,'C');
	$pdf->Ln();
}
$pdf->Output('','DSR/PWLDS'.'/'.date("d-m-Y").'.pdf');
