...
$pdf->Cell(200,5,'DATA SENSOR DI PIPA DINAS',0,1,'C');

$pdf->SetFont('Arial','',9);
$pdf->Cell(70,8,'Data Pipa Dinas Portal WLDS Sistem Monitoring Kota Cimahi',0,1,'L'); // cell dengan panjang 1
$pdf->ln(0.3);
$pdf->SetFont('Arial','',9);
$pdf->Cell(10,10,'NO',1,0,'C');
$pdf->Cell(40,10,'Debit Pipa Dinas Awal',1,0,'C');
$pdf->Cell(40,10,'Debit Pipa Dinas Akhir',1,0,'C');
$pdf->Cell(40,10,'Waktu',1,0,'C');
$pdf->Cell(40,10,'Tanggal',1,0,'C');
$pdf->Ln();

$pdf->SetFont('Arial','',8);
$cari_p = mysqli_query($conn, "SELECT * FROM t_p_dinas");
$no=1;
while ($data = mysqli_fetch_array($cari_p)){
	if (mysqli_num_rows($cari_p)>=1){
		$pdf->Cell(10,5,$no++,1,0,'C');
		$pdf->Cell(40,5,$data['debit_dinas_1'].' Liter/detik',1,0,'C');
		$pdf->Cell(40,5,$data['debit_dinas_1'].' Liter/detik',1,0,'C');
		...
		$pdf->Cell(40,5,'Tidak Ada Data',1,0,'C');
	}
}
$pdf->Output('','DSPD/PWLDS'.'/'.date("d-m-Y").'.pdf');
